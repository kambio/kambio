...
/*
 * mer_twist : A Java implementation of the MT19937 (Mersenne Twister)
 ...
 * 
 */
public class mer_twist extends Random {

	/**
	 * Auto-generated serial version UID.  Note that mer_twist does NOT
	...
	private int num_bytes_state = ((N + 1) * 4);

	// Temporary buffer used during setSeed(long)
	private transient int[] ibuf;

	/**
	 * The default constructor for an instance of mer_twist.  This invokes
	 ...
	 * behaviour.
	 */
	public mer_twist(boolean compatible) {
		super(0L);
		compat = compatible;
	 ...
	 * @throws IllegalArgumentException if the buffer has zero length.
	 */
	public mer_twist(int[] buf) {
		super(0L);
		setSeed(buf);
	}

	public mer_twist(mer_twist mt1){
		super(0L);
		set_state(mt1.get_state());
	}
	 ...
	 * is passed to two different instances of mer_twist (both of
	...
	protected final synchronized int next(int bits) {
		...
		if (mti >= N) {             // generate N words at one time
			...
			// Use the constructor mer_twist(true) to enable backwards
	...
	get_state(){
		...
		byte[] arr = new byte[num_bytes_state];
		ByteBuffer buff = ByteBuffer.wrap(arr);
		for(int aa = 0; aa < mt.length; aa++){
			buff.putInt(mt[aa]);
		}
		buff.putInt(mti);
		return arr;
	...
	public boolean
	set_state(byte[] ste){
		if(ste.length < num_bytes_state){ return false; }

		if(mt == null){ mt = new int[N]; }
		ByteBuffer buff = ByteBuffer.wrap(ste);
		for(int aa = 0; aa < mt.length; aa++){
			mt[aa] = buff.getInt();
		}
		mti = buff.getInt();
	...
	write_mer_twist_state(String f_nam, mer_twist pgen, byte[] pvk){
		byte[] ste = pgen.get_state();
		if(ste == null){ return false; }
		ste = bitshaker.encrypt_bytes(ste, pvk);
		if(ste == null){ return false; }
		ste = convert.bytes_to_hex_frm_bytes(ste, config.RAND_STR);
		if(ste == null){ return false; }
		//boolean w_ok = mem_file.write_bytes(new File(f_nam), ste);
		mem_file.concurrent_write_encrypted_bytes(new File(f_nam), null, ste);
		return true;
	}

	public static boolean
	read_mer_twist_state(String f_nam, mer_twist pgen, byte[] pvk){
		//byte[] ste = mem_file.read_bytes(new File(f_nam));
		byte[] ste = mem_file.concurrent_read_encrypted_bytes(new File(f_nam), null);
		if(ste == null){ return false; }
		ste = convert.hex_frm_bytes_to_bytes(ste, config.RAND_STR);
		if(ste == null){ return false; }
		ste = bitshaker.decrypt_bytes(ste, pvk);
		if(ste == null){ return false; }
		boolean ok1 = pgen.set_state(ste);
		return ok1;
	}
}
